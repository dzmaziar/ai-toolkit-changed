...
class BaseSDTrainProcess(BaseTrainProcess):
    ...
    def setup_adapter(self):
        ...
        if is_t2i:
        ...
        elif self.adapter_config.type == 'ip_diffusers_flux_kontext':
            pipe = getattr(self.sd, "pipeline", None)
            if pipe is None:
                raise ValueError("ip_diffusers_flux_kontext requires sd.pipeline to be initialized")
            if not hasattr(pipe, "load_ip_adapter"):
                raise ValueError(
                    "diffusers pipeline has no load_ip_adapter(); "
                    "pinned diffusers commit may not support Flux IP-Adapter"
                )
            if self.adapter_config.name_or_path is None:
                raise ValueError("ip_diffusers_flux_kontext requires adapter.name_or_path")
            if self.adapter_config.image_encoder_path is None:
                raise ValueError("ip_diffusers_flux_kontext requires adapter.image_encoder_path")

            load_kwargs = {}
            if self.adapter_config.weight_name is not None:
            ...
            print_acc(
                "Loading diffusers IP-Adapter for Flux-Kontext: "
                f"{self.adapter_config.name_or_path} "
                f"(weight_name={self.adapter_config.weight_name}, "
                f"image_encoder={self.adapter_config.image_encoder_path})"
            )
            try:
                pipe.load_ip_adapter(self.adapter_config.name_or_path, **load_kwargs)
            except TypeError:
                pipe.load_ip_adapter(
                    self.adapter_config.name_or_path,
                    weight_name=self.adapter_config.weight_name,
                    image_encoder_pretrained_model_name_or_path=self.adapter_config.image_encoder_path,
                )

            setattr(self.sd, "diffusers_ip_adapter_enabled", True)
            setattr(self.sd, "diffusers_ip_adapter_default_scale",
                    float(getattr(self.adapter_config, "conditioning_scale", 1.0)))
    ...
    def run(self):
        ...
        if not self.is_fine_tuning:
            ...
            if self.adapter_config is not None:
                ...
                if self.adapter is not None and self.train_config.gradient_checkpointing:
                    self.adapter.enable_gradient_checkpointing()
                flush()
