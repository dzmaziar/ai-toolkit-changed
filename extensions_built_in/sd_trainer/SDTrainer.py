...
from toolkit.util.tiling_losses_advanced import (
    ...
    MultiScaleSpectralLoss, LatentPeriodLoss, GlobalPeriodAnchorLoss
)
...
class SDTrainer(BaseSDTrainProcess):
    ...
    def __init__(self, process_id: int, job, config: OrderedDict, **kwargs):
        ...
        if self.train_config.training_mode == 'texture':
            tc = self.train_config
            self.local_texture_loss = LocalTextureRegularityLoss(
                patch_size=tc.local_patch_size, stride=tc.local_stride,
                min_coverage=tc.local_min_coverage,
                hard_k_frac=tc.local_hard_k_frac, hard_k_min=tc.local_hard_k_min,
                w_spectral=tc.local_w_spectral, w_afc=tc.local_w_afc,
                w_phase=tc.local_w_phase, w_log=tc.local_w_log,
                phase_center_weight=tc.center_weight, phase_pce_weight=tc.pce_weight,
                phase_temperature=tc.temperature, phase_use_hann=tc.use_hann,
                phase_demean=tc.demean, phase_exclude_radius=tc.exclude_radius,
                spectral_r_bins=tc.r_bins, spectral_tau=tc.tau,
                spectral_band_sigma=tc.band_sigma, spectral_w_peak=tc.w_peak,
                spectral_w_band=tc.w_band, spectral_min_bin=tc.min_bin,
                spectral_max_bin=tc.max_bin,
                afc_r_bins=tc.afc_r_bins, afc_tau=tc.afc_tau,
                afc_min_rel=tc.afc_min_rel, afc_max_rel=tc.afc_max_rel, afc_w=tc.afc_w,
                log_out_r=tc.log_out_r, log_out_t=tc.log_out_t,
                log_r_min=tc.log_r_min, log_w=tc.log_w,
            )
            self.phase_loss = PhaseCorrelationLoss(
                tc.center_weight, tc.pce_weight, tc.temperature,
                tc.use_hann, tc.demean, tc.exclude_radius,
            )
            self.spectral_period_loss = SpectralPeriodLoss(
                tc.r_bins, tc.tau, tc.band_sigma,
                tc.w_peak, tc.w_band, tc.min_bin, tc.max_bin,
            )
            self.logpolar_loss = LogPolarAlignLoss(
                tc.log_out_r, tc.log_out_t, tc.log_r_min, tc.log_w,
            )
            self.acf_period_loss = ACFPeriodLoss(
                tc.afc_r_bins, tc.afc_tau, tc.afc_min_rel, tc.afc_max_rel, tc.afc_w,
            )
            ...
            self.seamless_loss = SeamlessLoss(
                edge_width=tc.seamless_edge_width,
                use_gradient=tc.seamless_use_gradient,
                multi_scale=tc.seamless_multi_scale,
                check_diagonals=tc.seamless_check_diagonals,
                diagonal_weight=tc.seamless_diagonal_weight, w=1.0,
            )
            self.gradient_period_loss = GradientPeriodLoss(
                r_bins=tc.r_bins, tau=tc.tau, band_sigma=tc.band_sigma, w=1.0,
            )
            self.cross_patch_loss = CrossPatchConsistencyLoss(
                patch_size=tc.cross_patch_size, stride=tc.cross_patch_stride,
                r_bins=64, tau=0.08, w=1.0,
            )
            self.global_period_anchor_loss = GlobalPeriodAnchorLoss(
                patch_size=tc.anchor_patch_size, stride=tc.anchor_stride,
                r_bins=128, tau=0.06, w=1.0,
            )
            self.multiscale_loss = MultiScaleSpectralLoss(
                scales=tc.multiscale_scales, r_bins=tc.r_bins, tau=tc.tau, w=1.0,
            )
            self.latent_period_loss = LatentPeriodLoss(r_bins=64, tau=0.08, w=1.0)
    ...
    def calculate_loss(
        ...
        # funcs.py editing losses (Flux Kontext: identity, geometry, overlay, contrastive, multi_neg)
        _any_funcs = (
            self.train_config.funcs_identity_weight > 0
            or self.train_config.funcs_geometry_weight > 0
            or self.train_config.funcs_overlay_weight > 0
            or self.train_config.funcs_contrastive_weight > 0
            or self.train_config.funcs_multi_neg_weight > 0
        )
        _funcs_enabled = (
            _any_funcs
            and self.train_config.training_mode == 'object'
            and getattr(self.sd, 'arch', None) == 'flux_kontext'
            and batch.control_tensor is not None
        )
        if _funcs_enabled:
            # use only target part of noisy_latents when control is concatenated (32 ch)
            with torch.no_grad():
                noisy_target = noisy_latents[:, :16] if noisy_latents.shape[1] == 32 else noisy_latents
            bs = noise_pred.shape[0]
            stepped_latents = None
            if hasattr(self.sd, 'get_stepped_pred'):
                try:
                    stepped_latents = self.sd.get_stepped_pred(noise_pred, noise)
                except Exception:
                    stepped_latents = None
            if stepped_latents is None:
                noise_pred_chunks = torch.chunk(noise_pred, bs)
                timestep_chunks = torch.chunk(timesteps, bs)
                noisy_chunks = torch.chunk(noisy_target, bs)
                stepped_chunks = []
                for idx in range(bs):
                    model_output = noise_pred_chunks[idx]
                    timestep = timestep_chunks[idx]
                    self.sd.noise_scheduler._step_index = None
                    self.sd.noise_scheduler._init_step_index(timestep)
                    sample = noisy_chunks[idx].to(torch.float32)
                    sigma = self.sd.noise_scheduler.sigmas[self.sd.noise_scheduler.step_index]
                    sigma_next = self.sd.noise_scheduler.sigmas[-1]
                    prev_sample = sample + (sigma_next - sigma) * model_output
                    stepped_chunks.append(prev_sample)
                stepped_latents = torch.cat(stepped_chunks, dim=0)
            stepped_latents = stepped_latents.to(self.device_torch, dtype=dtype)
            # embeddings from latents: [B, C, H, W] -> [B, C*H*W]; z_pred keeps grad for backprop
            z_pred = stepped_latents.flatten(1)
            z_gt = batch.latents.to(self.device_torch, dtype=dtype).flatten(1).detach()
            with torch.no_grad():
                control_pixel = batch.control_tensor.to(self.device_torch, dtype=dtype)
                control_pixel = control_pixel * 2 - 1
                lat_h, lat_w = batch.latents.shape[2], batch.latents.shape[3]
                vae_scale = 2 ** (len(self.sd.vae.config.get('block_out_channels', [4, 4, 4, 4])) - 1)
                target_h, target_w = lat_h * vae_scale, lat_w * vae_scale
                if control_pixel.shape[2] != target_h or control_pixel.shape[3] != target_w:
                    control_pixel = F.interpolate(control_pixel, size=(target_h, target_w), mode='bilinear')
                control_list = [control_pixel[i] for i in range(control_pixel.shape[0])]
                control_latent = self.sd.encode_images(control_list)
                control_latent = control_latent.to(self.device_torch, dtype=dtype)
            z_src = control_latent.flatten(1).detach()
            overlay_tensor = getattr(batch, 'overlay_tensor', None)
            if overlay_tensor is not None and (self.train_config.funcs_overlay_weight > 0 or self.train_config.funcs_multi_neg_weight > 0):
                with torch.no_grad():
                    overlay_pixel = overlay_tensor.to(self.device_torch, dtype=dtype)
                    overlay_pixel = overlay_pixel * 2 - 1
                    if overlay_pixel.shape[2] != target_h or overlay_pixel.shape[3] != target_w:
                        overlay_pixel = F.interpolate(overlay_pixel, size=(target_h, target_w), mode='bilinear')
                    overlay_list = [overlay_pixel[i] for i in range(overlay_pixel.shape[0])]
                    overlay_latent = self.sd.encode_images(overlay_list)
                    overlay_latent = overlay_latent.to(self.device_torch, dtype=dtype)
                z_overlay = overlay_latent.flatten(1).detach()
            else:
                z_overlay = (0.5 * z_src + 0.5 * z_gt).detach()
            margin = self.train_config.funcs_margin
            kind_geom = self.train_config.funcs_geometry_kind
            if self.train_config.funcs_identity_weight > 0:
                additional_loss += self.train_config.funcs_identity_weight * loss_no_edit_identity(
                    z_pred, z_gt, z_src, margin=margin
                )
            if self.train_config.funcs_geometry_weight > 0:
                additional_loss += self.train_config.funcs_geometry_weight * loss_geometry_match(
                    z_pred, z_gt, kind=kind_geom
                )
            if self.train_config.funcs_overlay_weight > 0:
                additional_loss += self.train_config.funcs_overlay_weight * loss_overlay_avoidance(
                    z_pred, z_gt, z_overlay, margin=margin
                )
            if self.train_config.funcs_contrastive_weight > 0:
                # y=1: pull pred to gt; y=0: push pred from src
                B = z_pred.shape[0]
                z1_c = torch.cat([z_pred, z_pred], dim=0)
                z2_c = torch.cat([z_gt, z_src], dim=0)
                y_c = torch.cat([
                    torch.ones(B, device=z_pred.device, dtype=z_gt.dtype),
                    torch.zeros(B, device=z_pred.device, dtype=z_gt.dtype),
                ], dim=0)
                additional_loss += self.train_config.funcs_contrastive_weight * contrastive_pair_loss(
                    z1_c, z2_c, y_c, margin=self.train_config.funcs_margin,
                    dist=self.train_config.funcs_contrastive_dist,
                )
            if self.train_config.funcs_multi_neg_weight > 0:
                z_negs_stacked = torch.stack([z_src, z_overlay], dim=1)
                additional_loss += self.train_config.funcs_multi_neg_weight * multi_negative_ranking_loss(
                    z_pred, z_gt, z_negs_stacked, margin=margin,
                )
        ...
        # ── Texture / spectral losses (training_mode == 'texture') ──
        if (
            self.train_config.training_mode == 'texture'
            and not self.train_config.train_turbo
            and self.train_config.texture_loss in (
                "custom", "SpectralPeriodLoss", "LogPolarAlignLoss",
                "ACFPeriodLoss", "Phase", "local",
            )
        ):
            step = self.step_num
            ...
            if gate_mean_val >= 0.01 and final_beta >= 0.01:
                vae = self.sd.vae
                ...
                if model_out.shape[1] >= c_vae:
                    ...
                    v = torch.cat([model_out, torch.zeros(
                        model_out.shape[0], pad_ch, model_out.shape[2], model_out.shape[3],
                        device=model_out.device, dtype=model_out.dtype)], dim=1)

                v_scale = self.train_config.v_scale
                pred_x0_latents_tex = (x_t - t01 * (v_scale * v)).to(self.device_torch, dtype=vae.dtype)

                tex_loss = torch.zeros((), device=loss.device, dtype=loss.dtype)

                if getattr(self.train_config, "texture_loss_in_latents", False):
                    H_lat, W_lat = pred_x0_latents_tex.shape[-2], pred_x0_latents_tex.shape[-1]
                    if batch.mask_tensor is not None and batch.mask_tensor.ndim == 4:
                        mask_tex = batch.mask_tensor.to(vae.device, dtype=torch.float32)
                        mask_tex = torch.nn.functional.interpolate(mask_tex, size=(H_lat, W_lat), mode="nearest")
                    else:
                        mask_tex = torch.ones((pred_x0_latents_tex.shape[0], 1, H_lat, W_lat),
                                              device=vae.device, dtype=torch.float32)

                    def _latent_signal(z):
                        zf = z.float()
                        dims = tuple(range(1, zf.ndim))
                        mean = zf.mean(dim=dims, keepdim=True)
                        std = zf.std(dim=dims, keepdim=True)
                        return torch.tanh((zf - mean) / (std + 1e-6))

                    pred_sig = _latent_signal(pred_x0_latents_tex.squeeze(2) if pred_x0_latents_tex.ndim == 5 else pred_x0_latents_tex)
                    with torch.no_grad():
                        tgt_sig = _latent_signal(tgt_latents_vae.squeeze(2) if tgt_latents_vae.ndim == 5 else tgt_latents_vae)
                    tex_loss = self._compute_texture_losses(pred_sig, tgt_sig, mask_tex, step)
                else:
                    ...
                    if batch.mask_tensor is not None and pred_imgs.ndim == 4:
                        mask_tex = batch.mask_tensor.to(self.device_torch, dtype=torch.float32)
                        mask_tex = torch.nn.functional.interpolate(mask_tex, size=pred_imgs.shape[-2:], mode="nearest")
                        mask_tex = (mask_tex > 0.5).float()
                    elif pred_imgs.ndim == 4:
                        mask_tex = torch.ones(pred_imgs.shape[0], 1, pred_imgs.shape[2], pred_imgs.shape[3],
                                              device=self.device_torch, dtype=torch.float32)
                    else:
                        mask_tex = None
                    if mask_tex is not None:
                        tex_loss = self._compute_texture_losses(pred_imgs, tgt_imgs, mask_tex, step)

                additional_loss += (final_beta * gate_mean) * tex_loss

                if step == 1 or step == 2 or step % 50 == 0:
                    print(f"[step {step}] tex: beta={final_beta:.3f} gate={gate_mean_val:.3f} "
                          f"base={loss.item():.4f} tex={tex_loss.item():.4f}")
    ...
    def _compute_texture_losses(self, pred, tgt, mask, step):
        """Dispatch texture/spectral losses based on texture_loss config."""
        tc = self.train_config
        result = torch.zeros((), device=pred.device, dtype=pred.dtype)

        if tc.texture_loss == "custom":
            if tc.loss_local_coef > 0:
                result = result + tc.loss_local_coef * self.local_texture_loss(pred, tgt, mask)
            if tc.loss_Spect_coef > 0:
                ft = self.spectral_period_loss.get_peak_center(tgt, mask)
                result = result + tc.loss_Spect_coef * self.spectral_period_loss(pred, tgt, mask, center_override=ft)
            if tc.loss_Log_coef > 0:
                result = result + tc.loss_Log_coef * self.logpolar_loss(pred, tgt, mask)
            if tc.loss_AFC_coef > 0:
                result = result + tc.loss_AFC_coef * self.acf_period_loss(pred, tgt, mask)
            if tc.loss_Phase_coef > 0:
                result = result + tc.loss_Phase_coef * self.phase_loss(pred, tgt, mask)
            if tc.loss_seamless_coef > 0:
                result = result + tc.loss_seamless_coef * self.seamless_loss(pred, tgt, mask)
            if tc.loss_gradient_coef > 0:
                result = result + tc.loss_gradient_coef * self.gradient_period_loss(pred, tgt, mask)
            if tc.loss_cross_patch_coef > 0:
                result = result + tc.loss_cross_patch_coef * self.cross_patch_loss(pred, tgt, mask)
            if tc.loss_multiscale_coef > 0:
                result = result + tc.loss_multiscale_coef * self.multiscale_loss(pred, tgt, mask)
            if tc.loss_anchor_coef > 0:
                result = result + tc.loss_anchor_coef * self.global_period_anchor_loss(pred, tgt, mask)
        elif tc.texture_loss == "Phase":
            result = self.phase_loss(pred, tgt, mask)
            if step % 50 == 0:
                diag = self.phase_loss.diagnostics(pred, tgt, mask)
                print(f"[step {step}] PCL diag: center_prob={diag['center_prob']:.3f}  PCE={diag['PCE']:.2f}")
        elif tc.texture_loss == "SpectralPeriodLoss":
            ft = self.spectral_period_loss.get_peak_center(tgt, mask)
            result = self.spectral_period_loss(pred, tgt, mask, center_override=ft)
        elif tc.texture_loss == "LogPolarAlignLoss":
            result = self.logpolar_loss(pred, tgt, mask)
        elif tc.texture_loss == "ACFPeriodLoss":
            result = self.acf_period_loss(pred, tgt, mask)
        elif tc.texture_loss == "local":
            result = self.local_texture_loss(pred, tgt, mask)
