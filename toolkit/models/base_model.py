        ...
        with network:
            with torch.no_grad():
                ...
                for i in tqdm(range(len(image_configs)), desc=f"Generating Images", leave=False):
                    ...
                    if getattr(self, "diffusers_ip_adapter_enabled", False) and hasattr(pipeline, "prepare_ip_adapter_image_embeds"):
                        import inspect
                        from PIL import Image as PILImage
                        from PIL.ImageOps import exif_transpose

                        if gen_config.adapter_image_path is not None:
                            ip_img = exif_transpose(PILImage.open(gen_config.adapter_image_path)).convert("RGB")
                        else:
                            ip_img = PILImage.new("RGB", (int(gen_config.width), int(gen_config.height)), (0, 0, 0))

                        scale = getattr(gen_config, "adapter_conditioning_scale", None)
                        if scale is None:
                            scale = float(getattr(self, "diffusers_ip_adapter_default_scale", 1.0))
                        try:
                            scale = float(scale)
                        except Exception:
                            scale = 1.0
                        ...
                        fn = pipeline.prepare_ip_adapter_image_embeds
                        sig = inspect.signature(fn)
                        call_kwargs = {}
                        if "ip_adapter_image" in sig.parameters:
                            call_kwargs["ip_adapter_image"] = [ip_img]
                        if "ip_adapter_image_embeds" in sig.parameters:
                            call_kwargs["ip_adapter_image_embeds"] = None
                        if "device" in sig.parameters:
                            call_kwargs["device"] = self.device_torch
                        if "num_images_per_prompt" in sig.parameters:
                            call_kwargs["num_images_per_prompt"] = 1
                        if "do_classifier_free_guidance" in sig.parameters:
                            do_cfg = bool(gen_config.guidance_scale is not None and gen_config.guidance_scale > 1.0)
                            call_kwargs["do_classifier_free_guidance"] = do_cfg

                        ip_embeds = fn(**call_kwargs)

                        joint = extra.get("joint_attention_kwargs", None)
                        if joint is None:
                            joint = {}
                            extra["joint_attention_kwargs"] = joint
                        joint["ip_adapter_image_embeds"] = ip_embeds
